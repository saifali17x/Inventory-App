...
\restrict RjZSfURA1iPEw41wJJI4N8wPWA1g3meIMhNJDNvWNLbZM5yBi7PG3UmWGygdCaK
...
--
-- Name: calculateoverduefines(numeric); Type: FUNCTION; Schema: public; Owner: seezy
--

CREATE FUNCTION public.calculateoverduefines(p_fine_per_day numeric) RETURNS text
    LANGUAGE plpgsql
    AS $$
BEGIN
    UPDATE Transactions 
...
ALTER FUNCTION public.calculateoverduefines(p_fine_per_day numeric) OWNER TO seezy;

--
-- Name: getbookavailabilityreport(); Type: FUNCTION; Schema: public; Owner: seezy
--

CREATE FUNCTION public.getbookavailabilityreport() RETURNS TABLE(title character varying, first_name character varying, last_name character varying, category_name character varying, total_copies integer, copies_available integer, copies_issued integer)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION public.getbookavailabilityreport() OWNER TO seezy;

--
-- Name: getmemberhistory(integer); Type: FUNCTION; Schema: public; Owner: seezy
--

CREATE FUNCTION public.getmemberhistory(p_member_id integer) RETURNS TABLE(transaction_id integer, title character varying, first_name character varying, last_name character varying, issue_date date, due_date date, return_date date, fine_amount numeric, status public.transaction_status_enum)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
...
ALTER FUNCTION public.getmemberhistory(p_member_id integer) OWNER TO seezy;

--
-- Name: issuebook(integer, integer, integer, integer); Type: FUNCTION; Schema: public; Owner: seezy
--

CREATE FUNCTION public.issuebook(p_member_id integer, p_book_id integer, p_staff_id integer, p_loan_days integer) RETURNS text
    LANGUAGE plpgsql
    AS $$
DECLARE
...
ALTER FUNCTION public.issuebook(p_member_id integer, p_book_id integer, p_staff_id integer, p_loan_days integer) OWNER TO seezy;

--
-- Name: returnbook(integer, numeric); Type: FUNCTION; Schema: public; Owner: seezy
--

CREATE FUNCTION public.returnbook(p_transaction_id integer, p_fine_amount numeric) RETURNS text
    LANGUAGE plpgsql
    AS $$
DECLARE
...
ALTER FUNCTION public.returnbook(p_transaction_id integer, p_fine_amount numeric) OWNER TO seezy;
...
1	Bang-e-Dra	978-969-35-0001-1	1	1	7	1908	328	935.00	5	8
2	Naqsh-e-Faryadi	978-969-35-0002-8	2	2	1	1941	112	715.00	3	5
5	Yusuf bin Tashfin	978-969-35-0005-9	5	4	8	1963	256	979.00	6	9
7	Safina-e-Gham-e-Dil	978-969-35-0007-3	7	6	1	1969	223	858.00	8	12
8	Aag Ka Darya	978-969-35-0008-0	8	7	1	1959	387	1210.00	4	6
\.
...
9	Saif	Ali	saifalisalman4@gmail.com	03314054624	House number 23, Rehman Garden , Habibullah Road, Garhi Shahu, Lahore	2025-11-07	Public	Active
\.
...
SELECT pg_catalog.setval('public.members_member_id_seq', 9, true);
...
ALTER TABLE ONLY public.transactions
    ADD CONSTRAINT transactions_staff_id_fkey FOREIGN KEY (staff_id) REFERENCES public.staff(staff_id);
...
\unrestrict RjZSfURA1iPEw41wJJI4N8wPWA1g3meIMhNJDNvWNLbZM5yBi7PG3UmWGygdCaK
